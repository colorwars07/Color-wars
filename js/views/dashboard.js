 ...
 * Player Dashboard (Blindado con límite de 2MB)
 * ═══════════════════════════════════════════════════════
 */
...
function openRechargeModal() {
  ...
  showModal(`
    ...
    <p style="font-family:var(--font-mono);font-size:.62rem;color:var(--text-dim);margin-bottom:.5rem;letter-spacing:.05em;">PASO 3 — Sube el comprobante (Máx 2MB):</p>
    <div class="field-group">
...
async function submitRecharge() {
  ...
  // 🔥 AQUÍ ESTÁ EL CANDADO ANTI-FOTOS PESADAS (MÁXIMO 2MB) 🔥
  if (file.size > 2 * 1024 * 1024) {
    $err.textContent = 'El archivo es muy pesado (Máximo 2MB). Toma un capture de pantalla y súbelo.';
    return;
  }
  ...
  hideModal();
  showToast(`Solicitud enviada por $${usd.toFixed(2)}. Estado: Pendiente.`, 'warning', 6000);
}
