 ...
 * VERSIÓN FINAL BLINDADA - 100% COMPLETA
 * ═══════════════════════════════════════════════════════
 */
  ...
  if (window.CW_SESSION.matchId) {
    try {
      ...
      if (matchData) {
        ...
        // 🚨 ARREGLO 1: Reseteo de tiempo para que el primer turno no se pierda
...
// 🚨 ARREGLO 2: Seguro de explosión blindado para evitar congelamientos
async function _addMass(row, col, color) {
  ...
  try {
      ...
      console.error("Error crítico evitado:", error);
  } finally {
      _isAnimating = false; // Este "finally" garantiza que el tablero se desbloquee siempre
  }
}
